...
if (!img) throw '*❈↲ رد ع الصوره ال انت بدك تحطها للقروب يحب.*'
await conn.updateProfilePicture(m.chat, img).then(_ => m.reply('❈↲ *تم تغير صوره القروب*'))
} else throw '*❈↲ رد ع الصوره الي بدك تحطها للقروب يحب.*'}
handler.command = /^تغيير-الصوره|تغيرالصوره$/i
handler.group = true
handler.admin = true
